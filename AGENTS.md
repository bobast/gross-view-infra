...
This repository manages **infrastructure configuration** for the "gross-view" project. It contains Keycloak realm settings, Nginx reverse proxy configuration, VPN/secret/DNS infrastructure services (Tailscale, Vault, dnsmasq), and custom CSS styles for authentication forms.
...
- `docker-compose.yml` — Docker Compose stack: PostgreSQL, Keycloak, Nginx, n8n, Tailscale, Vault, dnsmasq
...
- IP map: postgres `172.28.0.2`, keycloak `172.28.0.3`, nginx `172.28.0.4`, n8n `172.28.0.5`, tailscale `172.28.0.6`, vault `172.28.0.7`, dns `172.28.0.8`.
- `172.20.x`, `172.19.x`, `172.18.x` are taken by the host/WSL and other Docker networks — avoid them.

### Tailscale (VPN)

- Auth key comes from the user's Tailscale Admin Console (`https://login.tailscale.com/admin/keys`) — it cannot be generated locally; store it as `TS_AUTHKEY` in `.env`.
- Requires `/dev/net/tun` and capabilities `NET_ADMIN` (+ `SYS_MODULE`); runs on WSL2/Linux hosts, not natively on Docker Desktop for Windows.
- The node appears in the tailnet as `gross-view-vpn`. Access control is governed by Tailscale ACLs in the Admin Console (not in this repo).
...
- Do not commit or log: `TS_AUTHKEY`, Vault unseal keys, Vault root token, or contents of `.env` / `certs/`
