...
This repo manages reverse proxy routing, Keycloak theme customizations, realm settings, VPN access, secrets storage, internal DNS, and multi-container orchestration. Nginx provides unified entry point routing `/sso` to Keycloak, `/api` to the **handler backend (gross-view-handler)**, and `/` to frontend. The handler runs its own **MCP-сервер** (`/api/mcp`), к которому обязателен подключён ИИ-агент opencode (см. `docs/opencode-mcp-integration.md`). WireGuard serves as the secure VPN channel to non-public services (Postgres, Keycloak admin, opencode); Vault stores the stack's credentials (used when migrating to cloud K8s).

## Infrastructure Services

### Static network

- Static network: every service has a fixed IP — do not change without updating `dns/dnsmasq.conf`. In docker-compose nginx (80/443), wireguard (51820/udp), vault (8200), keycloak (8080), dns (53), and **opencode (4096)** publish host ports; **postgres exposes no host port** (reachable only inside `keycloak_network` at `172.28.0.2`). **handler (gross-view-backend, порт 8082) в compose отсутствует** — он запускается в режиме отладки на хостовом компьютере, а nginx/opencode обращаются к нему через `host.docker.internal:8082`. In K8s opencode stays internal (ClusterIP, no LoadBalancer/hostPort).
...
- Do not commit or log: `WG_*` secrets in `.env`, `wireguard_config` generated peer/private keys, Vault unseal keys, Vault root token, `OPENCODE_AGENT_CLIENT_SECRET`, `OPENCODE_MCP_TOKEN`, or contents of `.env` / `certs/`
