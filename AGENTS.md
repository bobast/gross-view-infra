...
- `opencode/entrypoint.sh` — Точка входа контейнера `opencode` (docker): рендер `/root/.config/opencode/opencode.json` с remote MCP `gross-view`. **Авто-генерация токена**: если `OPENCODE_MCP_TOKEN` пуст, получает `client_credentials`-токен из Keycloak через `KEYCLOAK_INTERNAL_URL` + `OPENCODE_AGENT_CLIENT_ID`/`OPENCODE_AGENT_CLIENT_SECRET`. **Устойчивость**: попытки повторяются до `OPENCODE_TOKEN_MAX_ATTEMPTS` (default 5, пауза 2 s), причина отказа всегда логируется (`Response: …` — busybox wget печатает её даже с `-q`), и **рабочий `opencode.json` никогда не удаляется** при сбое — старый конфиг сохраняется с предупреждением (иначе transient-сбой Keycloak молча оставлял opencode без MCP, а UI показывал «серверы не обнаружены» без причины). Образ opencode — BusyBox, **`curl` в нём отсутствует**, поэтому используется `/usr/bin/wget` (нет TLS-стека → только внутренний plain-HTTP Keycloak). K8s-версия скрипта — `k8s/base/opencode-entrypoint-configmap.yaml` (обязана быть идентичной docker-версии)
