...
- TLS: in K8s the nginx pod terminates TLS using the `mint-box-tls` Secret. `k8s/base/tls-secret.yaml` creates a **bootstrap placeholder** `mint-box-tls` (self-signed cert for `mint-box.ru`, SAN includes `www.mint-box.ru`) so the nginx pod can start before certbot has run. The certbot CronJob then overwrites this Secret with the real Let's Encrypt certificate (`kubectl create secret tls ... --dry-run=client -o yaml | kubectl apply -f -`) and rolls the nginx Deployment. Do not store real certs in the repo.
