...
- `opencode/entrypoint.sh` — Точка входа контейнера `opencode` (docker): рендер `/root/.config/opencode/opencode.json` с remote MCP `gross-view` из `OPENCODE_MCP_URL`/`OPENCODE_MCP_TOKEN`
